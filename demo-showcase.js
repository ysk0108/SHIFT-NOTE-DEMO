(() => {
  ...
  function overview(){
    ...
    html+='<div class="spdemo-note"><b>デモの見方</b><br>実装済みの運用フローを架空データで再現しています。Web Push本番配信、自由記述の自動翻訳、労務・会社ルールを含む高度なシフト最適化は今後強化する領域です。</div><div class="spdemo-title"><h3>役職ごとに見る価値</h3><small>同じデータを違う視点で利用</small></div><div class="spdemo-tabs" style="grid-template-columns:1fr 1fr"><button class="on" onclick="SPDemo.persona(this,\'crew\')">クルー</button><button onclick="SPDemo.persona(this,\'manager\')">店長</button></div><div id="spdemoPersona">'+personaHtml('crew')+'</div>';
  ...
  const tourSlides=[
    ...
    ['7 / 8','店舗運用を一つにつなぐ','クルーと店長の店舗内運用に集中して見せる。','<div class="spdemo-card"><div class="name">店舗運用モード</div><div class="desc">現在のデモでは上位管理は非表示。クルーと店長の現場運用に絞っています。</div></div>'],
    ['8 / 8','SKILL PASSPORTが目指すもの','シフトアプリでも、マニュアルアプリでもない。人・店舗・日々の運営を一つにつなぐ。','<div class="spdemo-card"><div class="name">店舗オペレーション基盤</div><div class="desc">Skill Passport × Store Guide × シフト仮組み × マニュアル × 引き継ぎ</div><div class="value">「人が変わっても、店が変わっても、仕事の情報が途切れない。」</div></div>']
  ];
  ...
  function detail(key){
    if(['national','hq','store-settings','search','ops'].includes(key)){ setBody('<div class="spdemo-title"><h3>現在非表示</h3><small>上位管理</small></div><div class="spdemo-card"><div class="name">この機能は現在のデモでは表示していません</div><div class="desc">上位管理向けの基盤は残していますが、今回の説明対象から一旦外しています。</div></div><button class="spdemo-btn ghost" onclick="SPDemo.overview()">← 全機能ガイドへ</button>'); return; }
